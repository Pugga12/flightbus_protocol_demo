...
ssize_t UartCdcAcm::read(uint8_t *buffer, const size_t len) const {
    if (mDeviceReady) {
...
// returns -EIO if device not ready, number of bytes read if successful
ssize_t UartCdcAcm::write(const uint8_t *data, const size_t len) const {
    if (mDeviceReady) {
...
ssize_t UartCdcAcm::getAvailableRXBufferSize() {
    if (mDeviceReady) {
